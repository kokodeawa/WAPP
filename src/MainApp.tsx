...
export const MainApp: React.FC<MainAppProps> = ({ currentUser, onLogout }) => {
    ...
    const averageBudgetData = useMemo(() => {
        ...
        for (const budget of savedBudgets) {
            for (const category of budget.categories) {
                const existing = categorySums.get(category.id);
                if (existing) {
                    existing.sum += category.amount;
                } else {
                    categorySums.set(category.id, { name: category.name, sum: category.amount });
                }
            }
        }

        const averageCategories = Array.from(categorySums.entries()).map(([id, { name, sum }]) => ({
        id,
        name,
        amount: sum / savedBudgets.length,
        }));
  ...
  const handleConfirmForceCreate = () => {
    if (activeCycleConfig && periodStartDate && periodEndDate) {
      const allExpensesInCycle = { ...currentDailyExpenses };
  ...
  return (
    <>
      <Header activeTab={activeTab} onTabChange={setActiveTab} onMenuClick={() => setIsMenuOpen(true)} onAiClick={() => setIsAiAssistantOpen(true)} />
      ...
      <AiAssistant 
        ...
        averageBudgetData={averageBudgetData}
      />
    </>
  );
};